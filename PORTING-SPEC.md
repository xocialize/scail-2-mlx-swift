...
| **S2** | **full DiT forward ✅ PASSED 2026-06-20** (`--s2-dit-gate`: SCAILModel anim 1.7e-6 / replace 1.6e-6; 91 params load by natural keys, verify .all — re-proves key contract live). Remaining: segmented pipeline (wan-core FlowUniPC/DPM++ + WanVAE encode/decode + CFG loop) + CLIP bicubic preprocessing (matrices in Double); few-step e2e golden ≤0.05 | DiT ≤5e-3 ✅; e2e ≤0.05 | DiT done |
