...
| **S1** | net-new forwards vs oracle fixtures (CPU stream). **3-seg RoPE ✅** (`--s1-rope-gate`: freq-table reuse 0.0 + anim/replace 0.0; `RoPE3Seg` reuses wan-core `ropeParams`). **dual-mask Conv3d embeds ✅** (`--s1-patchembed-gate`: latent 20-ch + mask 28-ch, ~3e-7; `SCAILPatchEmbeds` = MLX-swift Conv3d). Remaining: **i2v cross-attn** (`k_img/v_img/norm_k_img` — split context [img‖text], two SDPA sharing q, summed), **CLIP ViT-H visual** (largest), wan-block reuse-sanity | ≤1e-4 RoPE / ≤1e-3 component (multi-frame temporal `<0.02` per family E16) | 2/4 done |
