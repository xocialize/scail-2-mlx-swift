...
| **S1** | net-new forwards vs oracle fixtures (CPU stream). **3-seg RoPE ✅ PASSED 2026-06-20** (`--s1-rope-gate`: freq-table reuse 0.0 + anim/replace application 0.0; `RoPE3Seg` reuses wan-core `ropeParams`, owns only the segment dispatch). Remaining: dual-mask Conv3d embeds, i2v cross-attn (`k_img/v_img`), CLIP ViT-H visual, wan-block reuse-sanity | ≤1e-4 RoPE / ≤1e-3 component (multi-frame temporal `<0.02` per family E16) | RoPE done |
